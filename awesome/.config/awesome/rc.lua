...
awful.screen.connect_for_each_screen(function(s)
    ...
    local names   = { "   Main   ", "   Dev   ", "   www   ", "   Comms   ", "   Other   " }
    ...
    local new_shape = function(cr, width, height)
        gears.shape.rounded_rect(cr, 47, 47, 5)
    end
            ...
            {
                {
                    ...
                    -- Commented out to hide text, only show icons
                    -- {
                    --     id     = 'text_role',
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey }, "space", function() awful.spawn("ulauncher") end,
...
-- *******************************************
-- **                                       **
-- **  Tag toggle/move client to tag/switch **
-- **                                       **
-- *******************************************
-- *******************************************
...
-- Left click sets active client
    awful.button({}, 1, function(c)
        c:emit_signal("request::activate", "mouse_click", { raise = true })
    end),
    -- MOD + Left click allows moving client
    awful.button({ modkey }, 1, function(c)
        c:emit_signal("request::activate", "mouse_click", { raise = true })
        awful.mouse.client.move(c)
    end),
    -- MOD + Right click allows resizing client
...
-- Handle plugging and unplugging of external monitors
...
-- the dock state changes and we erroneously attempt to restore the dead client
        ...
        for _, tag_list in pairs(tag_store) do
            for _, tag in pairs(tag_list) do
                for i, client in ipairs(tag) do
                    if c == client then
                        -- Remove client from any saved tags
                        table.remove(tag, i)
                        break
                    end
                end
...
--       s.mywibox:struts({top = 42 + beautiful.useless_gap * 2})
-- end)
